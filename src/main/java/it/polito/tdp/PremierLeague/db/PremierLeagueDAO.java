...
public class PremierLeagueDAO {
	...
	public void listMatches(int mese,Map<Integer,Match> idMap){
		String sql = "SELECT m.MatchID, m.TeamHomeID, m.TeamAwayID, m.teamHomeFormation, m.teamAwayFormation, m.resultOfTeamHome, m.date, t1.Name, t2.Name "
				+ "FROM Matches m, Teams t1, Teams t2 "
	...
	public List<Adiacenza> listAdiacenze(int mese,int minuti,Map<Integer,Match> idMap){
		String sql = "SELECT m1.MatchID AS id1,m2.MatchID AS id2,COUNT(DISTINCT a1.PlayerID) AS peso "
				...
				+ "WHERE a1.MatchID=m1.MatchID AND a2.MatchID=m2.MatchID AND a1.PlayerID=a2.PlayerID AND m1.MatchID>m2.MatchID AND MONTH(m1.Date)=? AND MONTH(m2.Date)=? AND a1.TimePlayed>=? AND a2.TimePlayed>=? "
